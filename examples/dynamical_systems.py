...
class DynamicalSystem:
    ...
    @abstractmethod
    def drift_func(self, x: tf.Tensor, u: tf.Tensor = None, **kwargs) -> tf.Tensor:
        """
    ...
    def forward(
        self, x: tf.Tensor, t: tf.Tensor, dt: tf.Tensor, noisy: bool = True, **kwargs
    ) -> tf.Tensor:
    ...
    def check_control(self, u: tf.Tensor = None):
        if not self.controllable:
            assert u is None, ValueError(
                f"{self.__class__} instance is not controllable."
            )
...
class ModelBasedSystem(DynamicalSystem):
    ...
    def check_control(self, u: tf.Tensor = None):
        if self.controllable:
            assert u is not None, ValueError(
                f"{self.__class__} instance requires control inputs."
            )
        else:
            assert u is None, ValueError(
                f"{self.__class__} instance is not controllable."
...
class HodgkinHuxley(DynamicalSystem):
    ...
    def drift_func(self, x: tf.Tensor, u: tf.Tensor = None, **kwargs) -> tf.Tensor:
        ...
        dv = (
            (0.0 if (u is None) else u / self.cM)
            + gK * (self.vK - v)
            + gNa * (self.vNa - v)
            + gL * (self.vL - v)
        )
