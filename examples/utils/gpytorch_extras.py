...
class GridGPR(gpflow.models.GPR):
    def __init__(
        ...
        self.gpytorch_model = self._create_gpytorch_model(
            _kernel, _likelihood, input_dim=input_dim, grid_size=grid_size
        )
        ...
        data = (
            convert_torchTensor(self.gpytorch_model.train_inputs[0]),
    ...
    def _create_gpytorch_model(cls, kernel, likelihood, input_dim, grid_size):
        ...
        grid_kernel = gpytorch.kernels.GridInterpolationKernel(
            kernel, num_dims=input_dim, grid_size=grid_size, grid_bounds=grid_bounds
        )
...
class GPyTorchSampler:
    ...
    def build(cls, model: GridGPR, sample_shape, **kwargs):
        return cls(model=model.gpytorch_model, sample_shape=sample_shape, **kwargs)
    ...
    def __call__(self, x):
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var(), gpytorch.settings.fast_pred_samples(), gpytorch.settings.max_root_decomposition_size(
            max_root
        ):
