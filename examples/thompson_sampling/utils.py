...
def take_along_axis(arr: tf.Tensor, indices: tf.Tensor, axis: int) -> tf.Tensor:
    ...
    for i, (size_a, size_i) in enumerate(zip(_arr.shape, _idx.shape)):
        if i == _axis:
            ...
            assert size_i in (1, size_a), ValueError(
                f"Shape mismatch: {_arr.shape} vs {_idx.shape}"
            )
...
def take_top_k(
    arr: tf.Tensor, vals: tf.Tensor, k: int, sign: str = "+", **kwargs
...
def find_top_k(
    fn: Callable[[tf.Tensor], tf.Tensor], batch_provider: Generator, k: int, **kwargs
) -> Tuple[tf.Tensor, tf.Tensor]:
    """
    ...
    for (
        i,
        xvals,
    ) in enumerate(map(tf.convert_to_tensor, batch_provider())):
        fvals = fn(xvals)
        ...
        if best_xvals.shape[-2] > k:
            best_xvals, best_fvals = take_top_k(
                arr=best_xvals, vals=best_fvals, k=k, **kwargs
            )
