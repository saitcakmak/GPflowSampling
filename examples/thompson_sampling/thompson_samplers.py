...
class ThompsonSampler(tf.Module):
    ...
    def __call__(
        self, num_choices: int, num_candidates: int = None, **kwargs
    ) -> tf.Tensor:
        ...
        **kwargs
        ...
        if batch_provider is None:
            ...
            batch_provider = build_batch_provider(
                num_batches=self.num_batches, batch_shape=batch_shape
            )
        ...
        top_xvals, top_fvals = find_top_k(
            fn=fn, batch_provider=batch_provider, k=num_candidates, sign="-"
        )
...
class PathwiseThompsonSampler(ThompsonSampler):
    ...
    def __call__(
        ...
        **kwargs
        ...
        result = optimizer.minimize(closure=closure, variables=[xvars], bounds=bounds)
