...
def _decoupled_sampler_svgp_mo_independent(
    ...
    **kwargs,
    ...
    return CompositeSampler(
        join_rule=join_rule, samplers=samplers, mean_function=model.mean_function
    )


@decoupled.register(gpflow.models.SVGP, LinearCoregionalization)
def _decoupled_sample_svgp_lcm(
    model: gpflow.models.SVGP,
    ...
    **kwargs,
    ...
    def join_rule(samples: List[tf.Tensor]) -> tf.Tensor:
        ...
        rhs = lhs_s[1:] + lhs_w[:1]
        return tf.einsum(
            f"{lhs_w},{lhs_s}->{rhs}", model.kernel.W, tf.squeeze(samples, axis=-1)
        )
