...
def _location_scale_gpr(
    ...
    return LocationScaleSamplerGPR(model=model, sample_shape=sample_shape, cache=cache)


@location_scale.register(gpflow.models.SVGP, gpflow.kernels.Kernel)
def _location_scale_svgp(
    ...
    return LocationScaleSamplerSVGP(model=model, sample_shape=sample_shape, cache=cache)
...
class LocationScaleSamplerGPR(Sampler):
    ...
    def __call__(
        self, X: TensorType, sample_shape: List[int] = None, full_cov: bool = None
    ) -> tf.Tensor:
        ...
        rvs = tf.random.normal(
            shape=list(sample_shape) + list(X.shape[:-1]), dtype=X.dtype
        )
        ...
        if full_cov:
            S = self.model.kernel(X, full_cov=True) - tf.matmul(
                iLuu_Kuf, iLuu_Kuf, transpose_a=True
            )
        ...
        v = self.model.kernel(X, full_cov=False) - tf.reduce_sum(
            tf.square(iLuu_Kuf), axis=-2
        )
    ...
    def build_cache(cls, model: gpflow.models.GPR):
        ...
        return CacheLocationScaleSamplerGPR(
            Z=Z, Luu=tf.linalg.cholesky(Suu), iLuu_err=iLuu_err
        )
...
class LocationScaleSamplerSVGP(Sampler):
    ...
    def __call__(
        ...
        if full_cov:
            S = (
                self.model.kernel(X, full_cov=True)
        ...
        v = self.model.kernel(X, full_cov=False) + tf.reduce_sum(
            tf.square(A) - tf.square(iLuu_Kuf), axis=-2
        )
        return m + tf.expand_dims(tf.sqrt(v) * rvs, axis=-1)
    ...
    def build_cache(cls, model: gpflow.models.SVGP):
        ...
        return CacheLocationScaleSamplerSVGP(
            Z=Z, Luu=tf.linalg.cholesky(Suu), q_mu=q_mu, q_sqrt=q_sqrt
        )
