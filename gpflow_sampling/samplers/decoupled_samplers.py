...
def _decoupled_sampler_gpr(
    ...
    def _create_prior_fn(batch_shape, basis: Callable = None):
        if basis is None:
            basis = RandomFourierBasis(
                kernel=model.kernel, units=num_basis, dtype=dtype
            )
    ...
    def _create_update_fn(batch_shape, prior_fn):
        ...
        def w_init(shape, dtype=dtype):
            prior_f = prior_fn(Z)
            prior_u = prior_f + (sigma2 ** 0.5) * tf.random.normal(
                prior_f.shape, dtype=prior_f.dtype
            )

            init = tf.linalg.adjoint(
                parallel_solve(
                    solver=tf.linalg.cholesky_solve, lhs=Luu, rhs=u - prior_u
                )
            )
    ...
    return CompositeSampler(
        join_rule=sum, samplers=[prior_fn, update_fn], mean_function=model.mean_function
    )


@decoupled.register(gpflow.models.SVGP, gpflow.kernels.Stationary)
def _decoupled_sampler_svgp(
    ...
    def _create_prior_fn(batch_shape, basis: Callable = None):
        if basis is None:
            basis = RandomFourierBasis(
                kernel=latent_kernel, units=num_basis, dtype=dtype
            )
    ...
    def _create_update_fn(batch_shape, prior_fn):
        ...
        def w_init(shape, dtype=dtype):
            ...
            prior_u = prior_f + (default_jitter() ** 0.5) * tf.random.normal(
                prior_f.shape, dtype=prior_f.dtype
            )
            ...
            init = tf.linalg.adjoint(
                parallel_solve(
                    solver=tf.linalg.cholesky_solve, lhs=Luu, rhs=induced_u - prior_u
                )
            )
    ...
    return CompositeSampler(
        join_rule=sum, samplers=[prior_fn, update_fn], mean_function=mean_function
    )
