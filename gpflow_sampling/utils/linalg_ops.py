...
# ==============================================
def parallel_solve(
    solver: Callable, lhs: tf.Tensor, rhs: tf.Tensor, **kwargs
) -> tf.Tensor:
    """
    ...
    if rank_diff == 0:
        ...
        x = solver(
            tf.broadcast_to(lhs, broadcast + shape_lhs[-2:]),
        ...
        _rhs = tf.broadcast_to(
            tf.convert_to_tensor(rhs)[rank_diff * (np.newaxis,)],
