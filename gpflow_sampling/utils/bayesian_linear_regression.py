...
class BayesianLinearRegression(BayesianModel):
    ...
    def _predict_f_from_data(
        ...
        **kwargs
        ...
        def solve_weight_space() -> tfd.Distribution:
            ...
            solv_y = parallel_solve(
                tf.linalg.triangular_solve, sqprec, proj, lower=True
            )
            solv_x = parallel_solve(
            ...
            if full_cov:
                ...
                return tfd.MultivariateNormalTriL(
                    loc=tf.squeeze(loc, axis=-1), scale_tril=scale_tril
                )
            else:
                scale_diag = tf.sqrt(tf.reduce_sum(tf.square(solv_x), axis=-2))
                return tfd.MultivariateNormalDiag(
                    loc=tf.squeeze(loc, axis=-1), scale_diag=scale_diag
                )

        def solve_function_space() -> tfd.Distribution:
            n_by_n = tf.matmul(
                x_train, weight_var[..., None, :] * x_train, transpose_b=True
            )
            precis = (1 / noise_precis) * eyeN + n_by_n
            sqprec = jitter_cholesky(precis)
            solv_x = parallel_solve(
                tf.linalg.triangular_solve, sqprec, x_train, lower=True
            )
            w_covar = solv_x * weight_var[..., None, :]  # scratch variable
            w_covar = tf.linalg.diag(weight_var) - tf.matmul(
                w_covar, w_covar, transpose_a=True
            )
    ...
    def predict_f_samples(
        ...
        **kwargs
        ...
        f_distrib = self.predict_f(
            predict_at, data=data, w_distrib=w_distrib, full_cov=full_cov, **kwargs
        )

        return f_distrib.sample(sample_shape)

    def predict_w(
        self, data: Tuple[tf.Tensor, tf.Tensor], use_weight_space: bool = None
    ) -> tfd.Distribution:
        """
        ...
        def solve_weight_space() -> tfd.Distribution:
            ...
            precis = tf.linalg.diag(1 / weight_var) + noise_precis * d_by_d
            sqprec = jitter_cholesky(precis)
            means = noise_precis * parallel_solve(
                tf.linalg.cholesky_solve, sqprec, proj
            )
        ...
        def solve_function_space() -> tfd.Distribution:
            ...
            covar = tf.linalg.diag(weight_var) - tf.matmul(
                covar, covar, transpose_a=True
            )
    ...
    def predict_w_samples(
        ...
        **kwargs
        ...
        if num_datum < num_weights:
            ...
            return self.update_w_samples(
                w_distrib=self.prior, w_samples=w_samples, data=data, **kwargs
            )
        else:
            w_distrib = self.predict_w(data=data, **kwargs)
            return w_distrib.sample(sample_shape=sample_shape)

    def update_w_samples(
        self,
        w_distrib: tfd.Distribution,
        w_samples: tf.Tensor,
        data: Tuple[tf.Tensor, tf.Tensor],
    ...
    def preprocess_data(
        self, data: Tuple[tf.Tensor, tf.Tensor], dtype: Any = None, **kwargs
